...
command = ('dnf install nasm dh-autoreconf.noarch openssl-devel fedora-packager rpmdevtools gcc systemd-devel zlib-devel -y')
print(command)
os.system(command)

command = ('rm -rf *.tar.gz && rm -rf *.tar.gz.*')
print(command)
os.system(command)

command = ('wget --user {0} --password {1} {2}').format(ulogin, upassword, download_url)
print(command)
os.system(command)
...
command = ('tar -zxf {0} -C ./QAT').format(http_file_k)
print(command)
os.system(command)

command = ('cd ./QAT && ./autogen.sh')
print(command)
os.system(command)

command = ('cd ./QAT && ./configure 2>&1 | tee configure.log  && echo $? | tee configure_r.log')
print(command)
os.system(command)

command = ('cd ./QAT && make rpm -j 2>&1 | tee make.log && echo $? | tee make_r.log ')
print(command)
os.system(command)
...
#     message2 = ("scp -r root:/var/lib/www/html/{0}/* .").format(dt)
#     print(message1, 'or' ,message2)



print('*********************************** End create RPMS script ****************************')
