    ...
    stages {
        ...
        stage('prereq'){
            steps{
                ...
                echo "STEP: Set fedora repo branch based on choosen OS"
                script{
                    switch(params.OS) {
                        case "fedora36rpm":
                        ...
                        case "Fedora38rpm":
                          ...
                          break
                        default:
                          env.FEDORA_KERNEL_BRANCH = "master";
                          break
                    }
        ...
        stage('build rpms'){
            steps {
                withCredentials([usernamePassword(credentialsId: "${env.artifactoryCreds}", passwordVariable: 'CREDS_PSW', usernameVariable: 'CREDS_USR')]){
                    sh '''
                        echo ${WORKSPACE}
                        chmod +x ./build_fedora_kernel_with_qat_patches_jenkins_version.sh
                        ./build_fedora_kernel_with_qat_patches_jenkins_version.sh ${QAT_PATCHES} ${FEDORA_KERNEL_BRANCH} ${BUILD_DIRECTORY} ${BUILD_SN} ${BUILD_STRING} ${BRANCH}
                    '''
                }
            }
        }
